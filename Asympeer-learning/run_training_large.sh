...
# =============== 数据与任务 ===============
DATA_PATH="${DATA_PATH:-clean_data_3}"        # 目录下需要有 train_split/ 和 val_split/
...
STAGE2_EPOCHS="${STAGE2_EPOCHS:-50}"             # 总计 ≈100，可按需改
...
if [[ "${USE_COMPILE}" == "1" ]]; then
  EXTRA_ARGS+=" --compile --compile_mode reduce-overhead"
else
...
python main.py \
  ...
  #--resume_step2_net2 "model/net2_step2_swin_base_patch4_window7_224_4775cls_last_1.pth"\
  #--resume_step2_net1 "model/net1_step2_convnext_base_4775cls_last_1.pth"\
  #--start_epoch 24\
